...
export function createToggleVoteMutation(voteType: VoteType) {
    return mutation({
        ...
        handler: async (ctx, args) => {
            ...
            const exsistingVote = await ctx.db
            .query(voteType)
            ...
            if (exsistingVote) {
                await ctx.db.delete(exsistingVote._id);
                await counts.dec(ctx, voteKey(args.postId, voteType))
                return
            }

            const exsistingOppositeVote = await ctx.db
            .query(oppositeVoteType)
            .withIndex("byPost", (q) => q.eq("postId", args.postId))
            .filter((q) => q.eq(q.field("userId"), user._id))
            .unique()

            if (exsistingOppositeVote) {
                await ctx.db.delete(exsistingOppositeVote._id);
                await counts.dec(ctx, voteKey(args.postId, oppositeVoteType))
            }

            await ctx.db.insert(voteType, {postId: args.postId, userId: user._id})
